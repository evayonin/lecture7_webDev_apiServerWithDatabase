...
public class DbUtils {
    ...
    // יצרתי את הדאטה בייס בפרויקט של הרצאה 7 צד שרת - שלבים:
    //קודם שי יצר דאטה בייס עם השם לוקל הוסט (לא שינה) ושם שם משתמש וססמה (אצלי זה root ומה שהגדרתי בהתחלה)
    //ואז בקונסולה יצר סכמה:
    //CREATE SCHEME ash_2025;
    //ואז מחק את הדאטה בייס הקודם (לחצן ימני ו remove data source ) כי רצה לעשות אחד חדש בתוך הסכמה אז עשה data source חדש עם mySQL והפעם ושם שוב אותם משתמש וססמה ובאינפוט של דאטה בייס כתב את שם הסכמה.
    //ואז אני יוצרת את טבלת היוזרים:
    //כפתור ימני על הסכמה -> new table -> לשנות שם -> כפתור ימני על columns -> new column -> לתת שם id , סוג דאטה int , לסמן תיבות not null, auto increment(רק בענידה הזאת, כי זה יהיה מפתח ראשי) -> לחצן ימני על id ב columns -> primary key.
    //נוסיף ככה עוד עמודות first_name, last_name, phone (כל אלה מסוג TEXT). ובסוף עמודה נוספת username מסוג VARCHAR(255) -> לחצן ימני עליו -> unique key (כי לא סומכים על מי שכתב את הקוד בצד שרת שלא יהיה יוזרניים שכבר קיים) -> OK.
    //לי עשה קצת בעיות עם הסינטקס אז מחקתי בpreview את השורה שעשה לי עליה שגיאה (יצר בטעות)
